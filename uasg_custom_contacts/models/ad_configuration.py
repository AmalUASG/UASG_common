...
class AdConfiguration(models.Model):
    ...
    def update_contacts(self) :
        ...
        req = req.json()
        access_token = req.get('access_token')
        get_members_url = str('https://graph.microsoft.com/v1.0/users?$filter=accountEnabled%20eq%20true&onPremisesLastSyncDateTime ge' + str(datetime.datetime.strptime(self.write_date,'%Y-%m-%dT%Hu%Mm%SsZ')) + '&$select=displayName,mail,id,mobilePhone,jobTitle,companyName,department&$top=999')
