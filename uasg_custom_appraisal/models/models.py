...
class Apprisal(models.Model):

    _name = 'appraisal'
    _inherit = [ 'mail.thread']

    name = fields.Char(tracking=True)
    employee = fields.Many2one('hr.employee',tracking=True)
    company_id = fields.Many2one('res.company')
    department = fields.Many2one('department')
    apprisal_duration_start = fields.Date(default=datetime.today(),tracking=True)
    apprisal_duration_end = fields.Date(tracking=True)
    survey_id = Many2one('survey.survey')


    @api.onchange('company_id')
    def get__departments(self):
        if self.company_id is True:
...
class Survey(models.Model):
    ...
    appraisal_id = fields.Many2one('appraisal')
