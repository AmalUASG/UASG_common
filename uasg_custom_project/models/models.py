...
class UASGProject(models.Model):
    ...
    def _compute_assign_to(self):
        ...
        for record in self :
            user_id = self.env['res.users'].search([('login','=',record.user_email)])
    ...
    def _compute_user_department(self):
        ...
        for record in self :
            ...
            if record.create_uid :
                ...
                if record.department == self.env.user.uasg_department :
...
#         #     raise UserError(str(response))
    ...
    def action_pipeline(self):
        ...
        if self.uasg_contact.manager_email == self.env.user.login :
            ...
            raise UserError('Sorry , Approval should be done by the Line Manager : ' + str(self.uasg_contact.manager_name))
    ...
    def action_reopen(self):
        ...
        if self.uasg_contact == self.env.user or self.self.uasg_contact.manager_email == self.env.user.login :
    ...
    def action_draft(self):
        ...
        if self.uasg_contact == self.env.user or self.uasg_contact.manager_email == self.env.user.login :
